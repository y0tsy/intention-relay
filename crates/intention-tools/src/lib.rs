...
#[cfg(test)]
const TEST_EXECUTE_TIMEOUT: Duration = Duration::from_millis(25);
...
#[allow(
    clippy::unwrap_used,
    reason = "Test fixtures use unwrap for compact fixed valid values."
)]
mod tests {
    use super::*;
    use std::io::Read as _;

    fn path() -> WorkspaceRelativePathDto {
        WorkspaceRelativePathDto::parse("src/lib.rs").unwrap()
    }
    ...
    #[test]
    fn registry_is_fixed_unique_and_stably_ordered() {
        let ids: Vec<_> = registry().into_iter().map(|d| d.id()).collect();
        assert_eq!(
            ids,
            [
                ToolId::Read,
    ...
    #[test]
    fn every_input_and_result_variant_round_trips() {
        let inputs = vec![
            ToolInput::Read(ReadInput { path: path() }),
        ...
        for input in inputs {
            ...
            assert_eq!(serde_json::from_str::<ToolInput>(&json).unwrap(), input);
        }
        let results = vec![
            ...
            ToolResult::Write(WriteResult { bytes: 4 }),
    ...
    #[test]
    fn bounded_text_rejects_nul_and_over_limit_but_accepts_boundary() {
        assert!(BoundedText::new("a\0b").is_err());
        assert!(BoundedText::new("a".repeat(1_048_576)).is_ok());
        assert!(BoundedText::new("a".repeat(1_048_577)).is_err());
    }

    #[test]
    fn bounded_lossy_truncates_at_utf8_boundary() {
        let bytes = vec![b'a'; MAX_TOOL_OUTPUT_BYTES + 2];
        let (text, truncated) = bounded_lossy(&bytes);
        assert!(truncated);
        assert!(text.ends_with("\n[truncated]"));
        let mut invalid = vec![b'a'; MAX_TOOL_OUTPUT_BYTES - 1];
        invalid.extend_from_slice("é".as_bytes());
        let (text, truncated) = bounded_lossy(&invalid);
        assert!(truncated);
        assert!(text.ends_with("\n[truncated]"));
    }

    #[test]
    fn bounded_reader_limits_output_and_reports_read_errors() {
        let mut reader = std::io::Cursor::new(vec![b'x'; MAX_TOOL_OUTPUT_BYTES + 1]);
        let mut output = Vec::new();
        assert!(read_bounded(&mut reader, &mut output).is_ok());
        ...
        assert_eq!(
            read_bounded(&mut Broken, &mut Vec::new()),
            Err("tool_execute_read_failed")
        );
        let mut output = Vec::new();
        assert_eq!(
            read_bounded(&mut std::io::Cursor::new(b"short"), &mut output),
            Ok(false)
        );
    ...
    #[test]
    fn bounded_helpers_preserve_existing_output_and_utf8() {
        let mut output = b"prefix".to_vec();
        assert_eq!(
            read_bounded(&mut std::io::Cursor::new(b"-suffix"), &mut output),
            Ok(false)
        );
        assert_eq!(output, b"prefix-suffix");
        let (value, truncated) = bounded_lossy("é".repeat(MAX_TOOL_OUTPUT_BYTES).as_bytes());
        assert!(truncated);
        assert!(value.ends_with("[truncated]"));
    }

    #[test]
    fn metadata_accessors_and_context_envelope_are_serializable() {
        let descriptor = registry()[0];
        assert_eq!(descriptor.id(), ToolId::Read);
        assert_eq!(descriptor.mutation(), MutationKind::ReadOnly);
        assert_eq!(descriptor.capabilities(), &[ToolCapability::Read]);
        ...
        assert_eq!(
            serde_json::from_str::<ToolResultEnvelope>(&serde_json::to_string(&envelope).unwrap())
                .unwrap(),
            envelope
        );
    ...
    #[test]
    fn bounded_output_captures_process_output() {
        ...
        command.stdout(Stdio::piped()).stderr(Stdio::piped());
        let output = bounded_output(command.spawn().unwrap(), CancellationSignal::new()).unwrap();
    ...
    #[test]
    fn cancellation_and_id_display_are_exercised() {
