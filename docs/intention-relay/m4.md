...
accepted Barrier 2 foundation is commit
`77e4ad5df4e3e598c5930a0ce03d2bbf624bcbda`, after controller integration of
Lane C (`3fdc9f4f20298db32d7b4463928f82ae6fe6ae21`) and Lane D
(`77e4ad5df4e3e598c5930a0ce03d2bbf624bcbda`). Lane C passed `make quick` and
`make verify`; the combined C+D integration passed
`TMPDIR=/home/data/.intention-relay-quality-tmp make quick` and
`TMPDIR=/home/data/.intention-relay-quality-tmp make verify`. Barrier 2
provides model execution lifecycle and opt-in run-stream protocol/client
foundations, but not application scheduling, daemon hosting, publisher fan-out,
or persistent subscriber delivery.
