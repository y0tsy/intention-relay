...
| Tool calls before M5 | Runtime records typed tool-call evidence then fails `tool_execution_unavailable` (superseded by ADR 0038 binding decision 2: the model-tool-loop executor requires a tool executor, and provider tool calls execute through the durable tool path). | Providers never invoke local tools through a retained no-tool-port fallback; no registry/WorkspaceRoot/hook behavior is pulled into M4. |
...
- compile/source architecture checks that restrict SDK namespaces to
