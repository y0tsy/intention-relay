...
| Stream lifecycle, policy, and durable facts | Model, runtime, domain, storage, and SQLite fixtures prove ordered stream facts, UTF-8-safe 4 KiB assistant batching, typed usage/finish/failure evidence, fixed 250 ms retry sequencing, timeout limits, tool-call denial before M5, atomic fact writes, cursor-bound replay, migration, and terminal fact rejection. The migration claim is historical: ADR 0038 removed the migration path, and storage now keeps one live schema version. |
