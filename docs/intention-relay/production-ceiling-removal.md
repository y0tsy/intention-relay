# Production ceiling removal — scope (TBD)

**Status:** draft scope reference. The concrete set of ceilings to remove is
not decided yet. The pull request for this work (PR #15) is the vehicle; the
list below is the candidate scope from the post-M4 production-limit audit and
must be confirmed before any cut is made.
...
| 3 | intention-domain | `ToolLifecycleEventDto.detail` inline cap | 4 KiB | ProductCeiling | |
| 4 | intention-domain | `MAX_TOOL_RESULT_CONTENT_BYTES` | 4 KiB | ProductCeiling | lib.rs L938, L1110 |
| 5 | intention-domain | `MAX_TOOL_RESULT_METADATA_ENTRIES` | 16 | ProductCeiling | lib.rs L940, L1112 |
| 6 | intention-domain | `MAX_TOOL_RESULT_METADATA_KEY_BYTES` | 128 | ProductCeiling | lib.rs L942, L1010 |
| 7 | intention-domain | `MAX_TOOL_RESULT_METADATA_VALUE_BYTES` | 1024 | ProductCeiling | lib.rs L944, L1011 |
| 8 | intention-domain | `ToolResultOutcomeDto::succeeded` reuses the 4-KiB content cap | 4 KiB | ProductCeiling | |
