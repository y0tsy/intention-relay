...
| RUN-001 | Execution kind is closed: Ordinary, Mandate, or VerifierMandate. | future | Adopt | DTO policy | Kind/version/payload mismatch blocks dependent external work. | Foundation | contract fixture planned |
| RUN-002 | Historical M3/M4 and ordinary records gain no synthetic future state. | historical-only | Adopt | DTO policy | Preserve readable history; do not reconstruct from current state. | Foundation | compatibility fixtures planned |
| MAN-001 | A Mandate is durable user-issued work authority, distinct from Goal, Skill, prompt, provider, and daemon. | mandate | Adopt | principles/scope | Non-authoritative references cannot schedule or mutate lifecycle. | Foundation | authority fixture planned |
| MAN-002 | Mandate revision changes affect only future fresh-run admission. | mandate | Adopt | sessions/storage | An admitted run retains immutable meaning. | Foundation | lifecycle fixture planned |
| MAN-003 | A Mandate continuation admits a new RunId and never resumes old work. | mandate | Adopt | sessions/storage | Restart never resumes provider, tool, process, kernel, child, MCP, or external work. | Foundation | recovery outcome planned |
...
| MAN-004 | Unknown terminal effect pauses the owning Mandate until exact reconciliation. | mandate | Adopt | sessions/storage | No retry, reattach, rediscovery, next step, or automatic continuation. | Foundation | crash matrix planned |
| MAN-005 | User lifecycle/revision commands win optimistic conflicts with daemon or verifier mutations. | mandate | Adopt | sessions/storage | Conflicting non-user mutation rejects safely. | Foundation | race fixture planned |
