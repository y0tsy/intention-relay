...
Active skills can add prompt guidance and constrain model-visible tools. The current UI has no direct live management interface and saving does not prove activation in the existing process.
...
User meaning: sub-agents are supporting workers initiated by the agent, observed and cancellable by the user.
