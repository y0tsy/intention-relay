...
| `make quality-self-test` | No | Prove isolated invalid fixtures fail their intended checks; `make verify` runs it after removing generated coverage artifacts. |
| `make quality-self-test-in-place` | No | Run the same fixture checks in place with `git restore` scoping and warm Cargo artifacts (the CI `ci-selftest` job). |
...
integration target to the machine-readable policy: `intention-daemon`
`real_api_e2e` (`crates/intention-daemon/tests/real_api_e2e.rs`). Ignored tests
are compiled by every test build but are never executed by `make test`,
`make coverage`, `make quick`, or `make verify`; executing them requires the
explicit `make e2e-real-api` opt-in with `INTENTION_REAL_API_KEY` and
`INTENTION_REAL_API_MODEL` (the target exports the `INTENTION_REAL_API_E2E=1`
flag itself, and optional `INTENTION_REAL_API_KIND`/`INTENTION_REAL_API_ENDPOINT`
select the provider kind and endpoint; a local gitignored `.env` file, when
present, supplies any of these variables that are not already set in the
environment). The manual
`.github/workflows/real-api-e2e.yml` workflow is `workflow_dispatch` only, is
