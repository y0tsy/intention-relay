...
The coverage policy declares an exact `coverage_targets` set for every coverage crate. The coverage runner narrows `--all-targets` to explicit `--lib`/`--bins`/`--tests` flags only when the declared set exactly equals the crate's locked Cargo metadata target inventory; otherwise it fails closed and keeps `--all-targets`. This preserves every production source file and required test harness in the report while avoiding unconditional target fan-out.
