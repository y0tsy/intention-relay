...
| `make test-default` | No | Run nextest suites and doctests for the default profile only (CI Windows split). |
| `make test-alt` | No | Run nextest suites and doctests for the no-default and all profiles (CI Windows split). |
...
| `make coverage-default-a` | No | Collect coverage for the default profile group a only (`run_coverage.py --profile default --group a`). |
| `make coverage-default-b` | No | Collect coverage for the default profile group b only (`run_coverage.py --profile default --group b`). |
| `make coverage-no-default-a` | No | Collect coverage for the no-default profile group a only (`run_coverage.py --profile no_default --group a`). |
| `make coverage-no-default-b` | No | Collect coverage for the no-default profile group b only (`run_coverage.py --profile no_default --group b`). |
| `make coverage-all-a` | No | Collect coverage for the all-features profile group a only (`run_coverage.py --profile all --group a`). |
| `make coverage-all-b` | No | Collect coverage for the all-features profile group b only (`run_coverage.py --profile all --group b`). |
...
| `make ci-test-default` | No | Run the CI test job for the default profile: job-scoped metrics, `test-default`, then job-scoped metrics finalize. |
| `make ci-test-alt` | No | Run the CI test job for the no-default and all profiles: job-scoped metrics, `test-alt`, then job-scoped metrics finalize. |
...
| `make ci-coverage-default-a` | No | Run the CI coverage job for the default profile group a: job-scoped metrics, `coverage-default-a`, generated-artifact cleanup, then job-scoped metrics finalize. |
| `make ci-coverage-default-b` | No | Run the CI coverage job for the default profile group b: job-scoped metrics, `coverage-default-b`, generated-artifact cleanup, then job-scoped metrics finalize. |
| `make ci-coverage-no-default-a` | No | Run the CI coverage job for the no-default profile group a: job-scoped metrics, `coverage-no-default-a`, generated-artifact cleanup, then job-scoped metrics finalize. |
| `make ci-coverage-no-default-b` | No | Run the CI coverage job for the no-default profile group b: job-scoped metrics, `coverage-no-default-b`, generated-artifact cleanup, then job-scoped metrics finalize. |
| `make ci-coverage-all-a` | No | Run the CI coverage job for the all-features profile group a: job-scoped metrics, `coverage-all-a`, generated-artifact cleanup, then job-scoped metrics finalize. |
| `make ci-coverage-all-b` | No | Run the CI coverage job for the all-features profile group b: job-scoped metrics, `coverage-all-b`, generated-artifact cleanup, then job-scoped metrics finalize. |
