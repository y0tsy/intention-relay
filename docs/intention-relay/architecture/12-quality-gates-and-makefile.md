...
The coverage runner always uses `--all-targets` for every coverage crate. Explicit target narrowing is intentionally disabled because explicit target sets do not reliably reproduce the `--all-targets` coverage set on every supported platform (Windows integration-target behavior differs), and per-crate thresholds must remain comparable across Linux and Windows.
