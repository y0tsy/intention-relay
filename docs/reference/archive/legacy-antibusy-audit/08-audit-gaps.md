...
4. Decide whether Headroom/CCR, VFR, prompt assembly, custom tools, hooks, skills, and MCP are v1 product commitments or advanced configuration.
...
- The frontend carries an `openPanel === 'files'` value but `TogglePanel.svelte` renders config, agents, logs, and history only.
...
The naming brief derives from proved interaction and observable behavior. It deliberately does not imply a promise of fully autonomous execution, perfect streaming reliability, live configuration application, direct MCP administration, or an LSP-based editor.
